...
func (m *CreatorLinkORM) ToPB(ctx context.Context) (CreatorLink, error) {
	...
	to.Type = protos.CreatorLinkType(m.Type)
...
func (m *StorageORM) ToPB(ctx context.Context) (Storage, error) {
	...
	to.Name = m.Name
	to.Description = m.Description
	to.Protocol = protos.StorageReferenceProtocol(m.Protocol)
	if m.Credentials != nil {
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedCreator && strings.HasPrefix(f, prefix+"Creator.") {
			...
			if patchee.Creator == nil {
				patchee.Creator = &protos.Creator{}
			}
			if o, err := protos.DefaultApplyFieldMaskCreator(ctx, patchee.Creator, patcher.Creator, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Creator.", db); err != nil {
				return nil, err
			} else {
				patchee.Creator = o
			}
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedCredentials && strings.HasPrefix(f, prefix+"Credentials.") {
			...
			if patchee.Credentials == nil {
				patchee.Credentials = &protos.StorageCredentials{}
			}
			if o, err := protos.DefaultApplyFieldMaskStorageCredentials(ctx, patchee.Credentials, patcher.Credentials, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Credentials.", db); err != nil {
				return nil, err
			} else {
		...
		if !updatedLocation && strings.HasPrefix(f, prefix+"Location.") {
			...
			if patchee.Location == nil {
				patchee.Location = &protos.StorageLocation{}
			}
